...
from .models import UserProfile, Student, Teacher, Course, Group, Attendance, Notification, Subject, Schedule, LeaveRequest, TimeSlot
...
def mark_schedule_attendance(request, group_id, period):
    try:
        ...
        return JsonResponse({'error': 'Колдонуучу профили табылган жок.'}, status=403)
...
def is_admin_manager_or_teacher(user):
    try:
        return user.userprofile.role in ['ADMIN', 'MANAGER', 'TEACHER']
    except:
        return False
...
def register(request):
    """
    Студент катталуу формасы.
    Бул жерде гана STUDENT роль автоматтык берилет,
    анткени бул студенттер үчүн гана катталуу формасы.
    """
    if request.method == 'POST':
        form = StudentRegistrationForm(request.POST)
        if form.is_valid():
            user = form.save()
            # Бул форма СТУДЕНТТЕР үчүн гана, ошондуктан STUDENT роль туура
            profile, created = UserProfile.objects.get_or_create(
                user=user,
                defaults={'role': 'STUDENT'}
            )
            # Эгер алда профил бар болсо, ролду STUDENT кылуу
            if not created:
            ...
            if parent_username:
                ...
                try:
                    # student.parents.add() автоматтык түрдө эки тараптан байланышты түзөт
                    student.parents.add(parent.userprofile)
                    messages.success(request, _('Parent linked successfully!'))
                except Exception as e:
                    messages.warning(request, _('Could not link parent profile.'))
...
def dashboard(request):
    ...
    from .statistics import get_unified_statistics
    ...
    if role in ['ADMIN', 'MANAGER']:
        # Биргелешкен статистика функциясын колдонуу
        stats = get_unified_statistics()
        
        # Dashboard үчүн керектүү маалыматтарды context'ке кошуу
        context.update({
            'total_students': stats['total_students'],
            'total_teachers': stats['total_teachers'],
            'total_groups': stats['total_groups'],
            'total_subjects': stats['total_subjects'],
            'groups_stats': stats['groups_stats'],
            'total_all_records': stats['total_all_records'],
        ...
        try:
            ...
            # Ар бир сабак үчүн attendance статусун кошуу
            today_schedule_with_status = []
            for schedule in today_schedule:
                # Бул сабак үчүн белгиленген attendance санын алуу
                marked_attendance_count = Attendance.objects.filter(
                    schedule=schedule,
                    date=today
                ).count()
                
                # Группадагы студенттердин жалпы саны
                total_students = Student.objects.filter(group=schedule.group).count()
                
                # Attendance статусу
                is_attendance_marked = marked_attendance_count > 0
                attendance_progress = (marked_attendance_count / total_students * 100) if total_students > 0 else 0
                
                # Schedule объектине кошумча маалыматтарды кошуу
                schedule.attendance_marked = is_attendance_marked
                schedule.marked_count = marked_attendance_count
                schedule.total_students = total_students
                schedule.attendance_progress = round(attendance_progress, 1)
                
                today_schedule_with_status.append(schedule)
            
            today_classes_count = today_schedule.count()
            
            # Белгиленген жана белгиленбеген сабактардын саны
            marked_classes_count = sum(1 for s in today_schedule_with_status if s.attendance_marked)
            unmarked_classes_count = today_classes_count - marked_classes_count
            ...
            context.update({
                ...
                'unmarked_classes_count': unmarked_classes_count,
                'today_schedule': today_schedule_with_status,
            })
        ...
        my_children = profile.parent_profiles.all()
...
def modern_dashboard(request):
    """
    Modern dashboard view with improved UI/UX inspired by Invilco design.
    Renders the modern_dashboard.html template with the same data as the regular dashboard.
    """
    from django.utils import timezone
    from datetime import datetime, timedelta
    ...
    # Күндөрдү Schedule моделинин форматына алмаштыруу
    weekday_map = {
        0: 'Monday',
        1: 'Tuesday',
        2: 'Wednesday',
    ...
    # TEACHER роль үчүн
        ...
        try:
            ...
            today_schedule = Schedule.objects.filter(
                ...
                day=today_weekday,
                is_active=True
            ).select_related('subject', 'group', 'time_slot').order_by('time_slot__order')
    ...
    # STUDENT роль үчүн
        ...
        excused_days = attendance_qs.filter(status='Excused').count()
        ...
        if student.group:
            student_today_schedule = Schedule.objects.filter(
                ...
                day=today_weekday,
                is_active=True
            ).select_related('subject', 'subject__teacher', 'time_slot').order_by('time_slot__order')
            
            # Катышуу статусун кошуу
            for schedule in student_today_schedule:
                try:
                    attendance_record = Attendance.objects.get(
                        student=student,
    ...
    # PARENT роль үчүн
        ...
        if my_children:
            ...
            messages.warning(request, _('Your child profile is not linked. Please contact administrator.'))
...
def mark_notification_read(request, notification_id):
    try:
        ...
        if notification.student in profile.parent_profiles.all():
...
def mark_group_attendance(request, schedule_id):
    try:
        ...
        messages.error(request, 'Колдонуучу профили табылган жок.')
        return redirect('dashboard')
...
def report(request):
    ...
    if report_type == 'daily':
        parsed_start_date = parsed_end_date = today
    elif report_type == 'weekly':
        week_start = today - timedelta(days=today.weekday())
        parsed_start_date = week_start
        parsed_end_date = today
    elif report_type == 'monthly':
        month_start = today.replace(day=1)
        parsed_start_date = month_start
        parsed_end_date = today
    ...
    # Группа боюнча статистика (Report форматында)
    group_stats = []
    for group_stat in stats['groups_stats']:
        group_stats.append({
            'group': {
                'id': group_stat['id'],
                'name': group_stat['name'],
                'course': group_stat['course']
            },
            'total': group_stat['total_records'],
            'present': group_stat['present_count'],
            'percentage': group_stat['attendance_rate']
        })
    ...
    if request.method == 'POST':
        ...
        if format_type == 'pdf':
            return export_detailed_pdf(stats['attendances'], {
            ...
            return export_detailed_excel(stats['attendances'])
    
    context = {
        'attendances': stats['attendances'].order_by('-date'),  # Show all records
        'total_records': stats['total_records'],
        'present_count': stats['present_count'],
        'absent_count': stats['absent_count'], 
        'late_count': stats['late_count'],
        ...
        'daily_trend': stats['weekly_attendance_data']['daily_trend_json'],
        'top_absent_students': stats['top_absent_students'],
...
def submit_leave_request(request):
    ...
    if user_profile.role == 'STUDENT':
        ...
        if linked_students.exists():
            ...
            messages.error(request, _('No student is linked to you.'))
            return redirect('dashboard')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, _('Your leave request has been successfully submitted. A notification has been sent to the administration.'))
...
@login_required
@user_passes_test(is_admin_manager_or_teacher)
...
def approve_leave_request(request, request_id):
    ...
    if request.method == 'POST':
        ...
        if action == 'approve':
            ...
            messages.success(request, _('Leave request for {student_name} has been approved. Notifications have been sent to the student and parents.').format(student_name=leave_request.student.name))
            ...
            messages.info(request, _('Leave request for {student_name} has been rejected. Notifications have been sent to the student and parents.').format(student_name=leave_request.student.name))
...
def advanced_report(request):
    ...
    if report_type == 'overview':
        ...
        if student_id:
            ...
            ).select_related('schedule__subject', 'schedule__teacher').order_by('-date', '-marked_at')
            ...
            # Сабактар боюнча статистика
            subject_breakdown = []
            subjects_attended = student_attendances.values('schedule__subject__id', 'schedule__subject__subject_name').distinct()
            
            for subj in subjects_attended:
                subj_attendances = student_attendances.filter(schedule__subject__id=subj['schedule__subject__id'])
                subj_stats = {
                    'subject_name': subj['schedule__subject__subject_name'],
                    'total': subj_attendances.count(),
                    'present': subj_attendances.filter(status='Present').count(),
                    'absent': subj_attendances.filter(status='Absent').count(),
                    'late': subj_attendances.filter(status='Late').count(),
                    'excused': subj_attendances.filter(status='Excused').count(),
                }
                if subj_stats['total'] > 0:
                    subj_stats['attendance_rate'] = round(
                        (subj_stats['present'] + subj_stats['late']) / subj_stats['total'] * 100, 1
                    )
                else:
                    subj_stats['attendance_rate'] = 0
                subject_breakdown.append(subj_stats)
            
            context['subject_breakdown'] = subject_breakdown
            
            # Күндөр боюнча тренд (соңку 30 күн)
            daily_attendance = {}
            for attendance in student_attendances:
                date_str = attendance.date.strftime('%Y-%m-%d')
                if date_str not in daily_attendance:
                    daily_attendance[date_str] = {'present': 0, 'absent': 0, 'late': 0, 'excused': 0}
                
                if attendance.status == 'Present':
                    daily_attendance[date_str]['present'] += 1
                elif attendance.status == 'Absent':
                    daily_attendance[date_str]['absent'] += 1
                elif attendance.status == 'Late':
                    daily_attendance[date_str]['late'] += 1
                elif attendance.status == 'Excused':
                    daily_attendance[date_str]['excused'] += 1
            
            # JSON форматка айландыруу (график үчүн)
            import json
            context['daily_attendance_json'] = json.dumps(daily_attendance)
        else:
            # Эгерде студент тандалбаса, бардык студенттердин жалпы статистикасын көрсөтөбүз
            all_students = Student.objects.all().order_by('group__name', 'name')
            ...
            for student in all_students:
                ...
                students_with_stats.append(stats)
            
            context['students_with_stats'] = students_with_stats
...
def view_student_profile(request, student_id):
    """
    Админ/Менеджер студенттин толук профилин жана катышуу тарыхын көрөт
    """
    # Уруксат текшерүү
    try:
        user_profile = UserProfile.objects.get(user=request.user)
        if user_profile.role not in ['ADMIN', 'MANAGER', 'TEACHER']:
            messages.error(request, 'Сизде бул бетке кирүүгө уруксат жок.')
            return redirect('dashboard')
    except UserProfile.DoesNotExist:
        messages.error(request, 'Профил табылган жок.')
        return redirect('dashboard')
    ...
    student_attendances = Attendance.objects.filter(
        student=student,
        date__range=[parsed_start_date, parsed_end_date]
    ).select_related('schedule__subject', 'schedule__teacher').order_by('-date', '-marked_at')
    ...
    context = {
        ...
        'daily_attendance_json': daily_attendance_json,
        'start_date': start_date,
        'end_date': end_date,
        'parents': parents,
    }
